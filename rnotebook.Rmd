...
2. Social support has been removed as not all previous data include such information.
3. The plot of lm.rank.1 is the opposite of lm.score.1.
4. By removing generosity and perceptions of corruption, we get similar R-squared values but a higher F-statistics.
...
data.2021 <- data.frame(GDP.per.capita=Logged.GDP.per.capita, #Social.support = Social.support,
                        Healthy.life.expectancy = Healthy.life.expectancy, Freedom=Freedom.to.make.life.choices)
lm.pred.score = predict(lm.score.2, data.2021) # no cv
glm.pred.score = predict(glm.score.multi, data.2021) # with cv
...
predicted <- subset(happy.covid, select=c(Country.name, lm.pred.score))
colnames(predicted)[2] <- "Pred_Score"
predicted <- predicted[order(predicted$Pred_Score),]
predicted$Pred_Rank <- 1:nrow(predicted)

results <- actual %>% inner_join(predicted, by = "Country.name")

# ggplot(NULL, aes(Rank, Country.name, group=Country.name)) +   
#  geom_point(data = actual, col = "red") +
#  geom_point(data = predicted, col = "blue")

# A few constants
#actualColor <- "#69b3a2"
#predictedColor <- rgb(0.2, 0.6, 0.9, 1)

# ggplot(results, aes(x=Country.name)) +
# geom_point( aes(y=Rank), size=2, color="blue") + 
# geom_point( aes(y=Pred_Rank), size=2, color="red") +
# scale_y_continuous(name = "Rank", sec.axis = sec_axis(name="Rank")) +theme_ipsum()
```
